...
public class FeiBanYaMaParser : BaseParser
{
    ...
    public FeiBanYaMaParser()
    {
        ...
        {
            new(
                @"\[Feibanyama\](?<title>[^\[\]]+?)-\s?(?<episode>\d+)(?:v(?<version>\d+))?\s?\[(?<websource>(?:CR|ABEMA|AMZN))\s(?<source>[a-zA-Z]+[Rr]ip)\s(?<resolution>\d+[pP])\s(?<codeV>HEVC-10bit)\s(?<codeA>AAC|OPUS|E-AC-3)\s(?<lang>.+?)\]",
                RegexOptions.IgnoreCase),
            new(
                @"\[沸班亚马制作组\](?<title>[^\[\]]+?)-\s?(?<episode>\d+)(?:v(?<version>\d+))?\s?\[(?<websource>(?:CR|ABEMA|AMZN))\s(?<source>[a-zA-Z]+[Rr]ip)\s(?:AI)?(?<resolution>\d+[pP])\s(?<codeV>HEVC-10bit)\s(?<codeA>AAC|OPUS|E-AC-3)\]\[(?<lang>.+?)\]",
                RegexOptions.IgnoreCase),
        };
    }
