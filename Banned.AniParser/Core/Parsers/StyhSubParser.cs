...
public partial class StyhSubParser : BaseParser
{
    ...
    [GeneratedRegex(@"\[霜庭云花Sub]\[(?<title>[^\[\]]+?)]\[(?<episode>\d+)(?:V(?<version>\d+))?(\sEND)?]\[(?<resolution>\d+p)]\[(?<codec>HEVC|AVC)\s?(?<codeA>AAC)?]\[(?<lang>.+?)]\[(?<source>[a-z]+Rip)]",
                    RegexOptions.IgnoreCase)]
    private static partial Regex SinglePattern1();

    [GeneratedRegex(@"\[霜庭云花Sub](?<title>[^\[\]]+?)-\s?(?<episode>\d+)(?:V(?<version>\d+))?\s?\[(?<source>[a-z]+Rip)\s(?<resolution>\d+p)\s(?<codec>HEVC|AVC)\s?(?<codeA>AAC)?]\[(?<lang>.+?)]",
                    RegexOptions.IgnoreCase)]
    ...
    [GeneratedRegex(@"\[STYHSub](?<title>[^\[\]]+?)-\s?S(?<season>\d+)E(?<episode>\d+)(?:v(?<version>\d+))?\s?-\s?\[(?<source>[a-z])\s(?<codec>HEVC|AVC)\s?(?<codeA>AAC)?\s(?<lang>.+?)\s?(V(?<version1>\d+))?]",
                    RegexOptions.IgnoreCase)]
    private static partial Regex SinglePattern4();

    [GeneratedRegex(@"\[霜庭云花Sub]\[(?<title>[^\[\]]+?)]\[(?<start>\d+)-(?<end>\d+)\s?(?<OAD>[a-z\u4e00-\u9fff]+)?]\[(?<resolution>\d+p)]\[(?<codec>HEVC|AVC)\s?(?<codeA>AAC)?]\[(?<lang>.+?)]\[(?<source>[a-z]+Rip)]",
                    RegexOptions.IgnoreCase)]
